...
> 装完后启动一个 codebuddy 执行器，并告诉我分配到的 agent_id。”
...
脚本会交互引导：发现队伍 → 匿名登记入队 → 启动执行器。非交互（自动化作答/脚本/CI）传参：

```powershell
powershell -ExecutionPolicy Bypass -File setup_worker_windows.ps1 `
  -Host 192.168.31.186 -Executor codebuddy
```
...
| `agent_bus/` | Python SDK（MQTT 客户端、discovery 发现协议、files、遗留 provision 凭据逻辑） |
...
- **仅限可信局域网**：任何能访问 `1883/8000` 端口的设备都可入队并读到全部消息。跨网/公网部署需恢复认证或加 TLS（见 `docs/broker_setup.md`）。
