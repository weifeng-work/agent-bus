...
环境变量:
  ...
  BUS_MQTT_USER    MQTT 用户名（= agent_id，broker 已禁匿名，必填）
  BUS_MQTT_PASS    MQTT 密码（由 broker 侧 scripts/add_node.py 发放）
  BUS_HTTP_TOKEN   HTTP API/文件服务令牌（由 scripts/add_node.py 发放）
"""
import os
