...
class TrayGui:
    ...
    def _menu(self):
        ...
        return pystray.Menu(
            ...
            pystray.MenuItem(
                "shell 受控能力（M3）", self._toggle_shell_control,
                checked=lambda item: n.shell_control, enabled=False),
            pystray.Menu.SEPARATOR,
            pystray.MenuItem("自修复", self._self_heal),
            pystray.MenuItem("查看错误日志", self._open_logs),
            pystray.MenuItem("查看受控记录（M3）", self._open_control_log, enabled=False),
